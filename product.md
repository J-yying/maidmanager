...
1.  **排班 (每日/每周)：** 店长与员工沟通后，在系统“拖拽”或“录入”该员工的**可服务时间段**（如：2023-11-19 14:00-22:00）。
2.  **接客/开单：**
      * 客人到店，店长查询“当前空闲人员”。
      * 选择员工 -\> 输入客户名（如“王总”）-\> 选择时长 -\> 确认金额 -\> **系统生成订单**。
...
### 模块三：业务开单 (Order Processing) —— **系统核心**
  ...
  * **F3.1 可用性查询 (Availability Check)：**
      * 在开单页面，输入`开始时间`和`时长`，系统自动筛选出满足以下条件的员工：
        1.  **在班：** 订单时间在排班时段内。
        2.  **空闲：** 订单时间与该员工已有的订单**无重叠**。
  * **F3.2 订单录入：**
...
1.  **数据一致性：** 严禁“撞单”。在并发情况下（虽然店长可能就一个人，但为了逻辑严密），数据库层面需保证同一员工在同一时间段只能有一条有效订单。
...
**场景：早退冲突检测**
  ...
  * **前提：** 员工A排班为 `14:00 - 22:00`。
  * **现状：** 已经有一个订单是 `21:00 - 22:00`。
  * **操作：** 店长试图将员工A的排班修改为 `14:00 - 20:00` (早退)。
  * **系统判定：**
    1.  查询员工A在 `20:00 - 22:00` (被切掉的时间段) 是否有订单？
